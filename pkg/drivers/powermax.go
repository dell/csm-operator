...
const (
	...
	CSIPmaxPollingInterval  = "<X_CSI_POLLING_INTERVAL>"
	...
	case "Node":
		if cr.Spec.Driver.Common != nil {
			for _, env := range cr.Spec.Driver.Common.Envs {
				...
				if env.Name == "X_CSI_SG_POLLING_INTERVAL" {
					pollingInterval = env.Value
				}
			}
		}
		...
		yamlString = strings.ReplaceAll(yamlString, CSIPmaxPollingInterval, pollingInterval)
	case "Controller":
		if cr.Spec.Driver.Common != nil {
			for _, env := range cr.Spec.Driver.Common.Envs {
				if env.Name == "X_CSI_MANAGED_ARRAYS" {
