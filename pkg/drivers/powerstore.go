...
const (
	...
	// PowerStoreConfigVolumeMount -
	PowerStoreConfigVolumeMount = "powerstore-config"
...
func ModifyPowerstoreCR(yamlString string, cr csmv1.ContainerStorageModule, fileType string) string {
	...
	fsckEnabled := GetDriverCommonEnv(cr, "X_CSI_FS_CHECK_ENABLED", "false")
	fsckMode := GetDriverCommonEnv(cr, "X_CSI_FS_CHECK_MODE", "checkOnly")
