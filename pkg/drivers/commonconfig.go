...
func GetController(ctx context.Context, cr csmv1.ContainerStorageModule, operatorConfig tools.OperatorConfig, driverName csmv1.DriverType) (*tools.ControllerYAML, error) {
	...
	driverYAML, err := tools.GetDriverYaml(YamlString, "Deployment")
	if err != nil {
		log.Errorw("GetController get Deployment failed", "Error", err.Error())
		return nil, err
	}

	controllerYAML := driverYAML.(tools.ControllerYAML)
	...
	for i, c := range containers {
		if c.Name != nil && string(*c.Name) == "driver" {
			...
			if cr.Spec.Driver.Common != nil || cr.Spec.Driver.Controller != nil {
				...
				containers[i].Env = tools.ReplaceAllApplyCustomEnvs(c.Env, commonEnvs, controllerEnvs)
		...
		if !removeContainer {
			tools.ReplaceAllContainerImageApply(operatorConfig.K8sVersion, &c)
			tools.UpdateSideCarApply(cr.Spec.Driver.SideCars, &c)
...
func GetNode(ctx context.Context, cr csmv1.ContainerStorageModule, operatorConfig tools.OperatorConfig, driverType csmv1.DriverType, filename string, ct client.Client) (*tools.NodeYAML, error) {
	log := logger.GetLogger(ctx)
	...
	driverYAML, err := tools.GetDriverYaml(YamlString, "DaemonSet")
	if err != nil {
		log.Errorw("GetNode Daemonset failed", "Error", err.Error())
		return nil, err
	}

	nodeYaml := driverYAML.(tools.NodeYAML)
	...
	for i, c := range containers {
		if c.Name != nil && string(*c.Name) == "driver" {
			...
			if cr.Spec.Driver.Common != nil {
				...
				if cr.Spec.Driver.Node != nil {
					containers[i].Env = tools.ReplaceAllApplyCustomEnvs(c.Env, cr.Spec.Driver.Common.Envs, cr.Spec.Driver.Node.Envs)
					c.Env = containers[i].Env
				}
			}
		}
		...
		if !removeContainer {
			tools.ReplaceAllContainerImageApply(operatorConfig.K8sVersion, &containers[i])
			tools.UpdateSideCarApply(cr.Spec.Driver.SideCars, &containers[i])
			newcontainers = append(newcontainers, c)
		}
	}
	...
	for i := range initcontainers {
		tools.ReplaceAllContainerImageApply(operatorConfig.K8sVersion, &initcontainers[i])
		tools.UpdateInitContainerApply(updatedCr.Spec.Driver.InitContainers, &initcontainers[i])
...
// GetUpgradeInfo -
func GetUpgradeInfo(ctx context.Context, operatorConfig tools.OperatorConfig, driverType csmv1.DriverType, oldVersion string) (string, error) {
	log := logger.GetLogger(ctx)
	...
	var upgradePath tools.UpgradePaths
...
func GetConfigMap(ctx context.Context, cr csmv1.ContainerStorageModule, operatorConfig tools.OperatorConfig, driverName csmv1.DriverType) (*corev1.ConfigMap, error) {
	log := logger.GetLogger(ctx)
...
func GetCSIDriver(ctx context.Context, cr csmv1.ContainerStorageModule, operatorConfig tools.OperatorConfig, driverName csmv1.DriverType) (*storagev1.CSIDriver, error) {
	log := logger.GetLogger(ctx)
