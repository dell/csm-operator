...
func ReverseProxyPrecheck(ctx context.Context, op tools.OperatorConfig, revproxy csmv1.Module, cr csmv1.ContainerStorageModule, r tools.ReconcileCSM) error {
...
func ReverseProxyServer(ctx context.Context, isDeleting bool, op tools.OperatorConfig, cr csmv1.ContainerStorageModule, ctrlClient crclient.Client) error {
	log := logger.GetLogger(ctx)
	YamlString, err := getReverseProxyDeployment(op, cr)
	if err != nil {
		return err
	}
	deployObjects, err := tools.GetModuleComponentObj([]byte(YamlString))
	if err != nil {
		return err
	}
...
func ReverseProxyStartService(ctx context.Context, isDeleting bool, op tools.OperatorConfig, cr csmv1.ContainerStorageModule, ctrlClient crclient.Client) error {
	log := logger.GetLogger(ctx)
...
func getReverseProxyService(op tools.OperatorConfig, cr csmv1.ContainerStorageModule) (string, error) {
	yamlString := ""
	...
	yamlString = strings.ReplaceAll(yamlString, tools.DefaultReleaseName, cr.Name)
	yamlString = strings.ReplaceAll(yamlString, ReverseProxyPort, proxyPort)
	yamlString = strings.ReplaceAll(yamlString, tools.DefaultReleaseNamespace, cr.Namespace)
...
func getReverseProxyDeployment(op tools.OperatorConfig, cr csmv1.ContainerStorageModule) (string, error) {
	YamlString := ""
	...
	YamlString = strings.ReplaceAll(YamlString, tools.DefaultReleaseNamespace, proxyNamespace)
	YamlString = strings.ReplaceAll(YamlString, ReverseProxyPort, proxyPort)
...
func ReverseProxyInjectDeployment(dp v1.DeploymentApplyConfiguration, cr csmv1.ContainerStorageModule, op tools.OperatorConfig) (*v1.DeploymentApplyConfiguration, error) {
...
func getRevProxyVolumeComp(revProxyModule csmv1.Module) []acorev1.VolumeApplyConfiguration {
	...
	isNewReverProxy, _ := tools.MinVersionCheck("v2.12.0", revProxyModule.ConfigVersion)
...
func getRevproxyApplyCR(cr csmv1.ContainerStorageModule, op tools.OperatorConfig) (*csmv1.Module, *acorev1.ContainerApplyConfiguration, error) {
	var err error
