...
func ResiliencyPrecheck(ctx context.Context, op tools.OperatorConfig, resiliency csmv1.Module, cr csmv1.ContainerStorageModule, _ tools.ReconcileCSM) error {
...
func ResiliencyInjectClusterRole(clusterRole rbacv1.ClusterRole, cr csmv1.ContainerStorageModule, op tools.OperatorConfig, mode string) (*rbacv1.ClusterRole, error) {
	var err error
...
func ResiliencyInjectRole(role rbacv1.Role, cr csmv1.ContainerStorageModule, op tools.OperatorConfig, mode string) (*rbacv1.Role, error) {
	...
	if resiliencyVersion == "" {
		resiliencyVersion, err = tools.GetModuleDefaultVersion(cr.Spec.Driver.ConfigVersion, cr.Spec.Driver.CSIDriverType, resiliencyModule.Name, op.ConfigDirectory)
		if err != nil {
			return nil, err
		}
	}
	isOldResiliencyVersion, err := tools.MinVersionCheck(resiliencyVersion, "v1.12.0")
	if err != nil {
		return nil, err
	}
...
func getResiliencyEnv(resiliencyModule csmv1.Module, _ csmv1.DriverType) string {
	for _, component := range resiliencyModule.Components {
		if component.Name == tools.PodmonNodeComponent {
...
func modifyPodmon(component csmv1.ContainerTemplate, container *acorev1.ContainerApplyConfiguration) {
	...
	container.Env = tools.ReplaceAllApplyCustomEnvs(container.Env, emptyEnv, component.Envs)
	container.Args = tools.ReplaceAllArgs(container.Args, component.Args)
}

func setResiliencyArgs(m csmv1.Module, mode string, container *acorev1.ContainerApplyConfiguration) {
	for _, component := range m.Components {
		if component.Name == tools.PodmonControllerComponent && mode == controllerMode {
			modifyPodmon(component, container)
		}
		if component.Name == tools.PodmonNodeComponent && mode == "node" {
			modifyPodmon(component, container)
		}
	}
}
...
func getResiliencyApplyCR(cr csmv1.ContainerStorageModule, op tools.OperatorConfig, driverType, mode string) (*csmv1.Module, *acorev1.ContainerApplyConfiguration, error) {
...
func ResiliencyInjectDeployment(dp applyv1.DeploymentApplyConfiguration, cr csmv1.ContainerStorageModule, op tools.OperatorConfig, driverType string) (*applyv1.DeploymentApplyConfiguration, error) {
...
func ResiliencyInjectDaemonset(ds applyv1.DaemonSetApplyConfiguration, cr csmv1.ContainerStorageModule, op tools.OperatorConfig, driverType string) (*applyv1.DaemonSetApplyConfiguration, error) {
