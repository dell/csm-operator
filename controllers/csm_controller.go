...
func (r *ContainerStorageModuleReconciler) checkUpgrade(ctx context.Context, cr *csmv1.ContainerStorageModule, operatorConfig utils.OperatorConfig) (bool, error) {
	...
	if configVersionExists {
		...
		isValidUpgrade, err := utils.IsValidUpgrade(ctx, oldVersion, newVersion, driverType, operatorConfig)
		if err != nil {
			return false, err
		}
		if !isValidUpgrade {
			return false, fmt.Errorf("invalid upgrade path")
		}
		// Call applyConfigVersionAnnotations during upgrade
		isUpdated := applyConfigVersionAnnotations(ctx, cr)
		if isUpdated {
			err = r.GetClient().Update(ctx, cr)
			if err != nil {
				log.Error(err, "Failed to update CR with annotation")
				return false, err
			}
